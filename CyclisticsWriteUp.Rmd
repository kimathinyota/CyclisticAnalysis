...
flextable(schema %>% mutate(completion_rate=(total_rows-n_missing)/total_rows) %>% mutate(across(where(is.numeric), round, 3)))
```
```{r Loading large CSV file, include=FALSE}
folder <- "~/Data Science/Coursera Google Analytics Course/CyclisticsAnalysis"
path_to_data <- file.path(folder, "2022-202303-divvy-tripdata.csv")
...
path_to_data <- file.path(folder, "clean_2022-202303-divy-tripdata.csv")
...
```{r Save basic summary, include=FALSE}
summary_path <- file.path(folder, "basic_summary.csv")
write.csv(basic_summary, summary_path, row.names=FALSE)
```
...
path_to_end_data <- file.path(folder, "end_location_2022-202303-divy-tripdata.csv")
path_to_start_data <- file.path(folder, "start_location_2022-202303-divy-tripdata.csv")
...
library(readr) 
path_to_community_data <- file.path(folder, "chicago_location_data_by_community.csv")
